...
export default function DomainsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg bg-[#161922] border border-[#232736] overflow-hidden">
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-[#232736]">
            {domains.map((dom) => (
              <tr key={dom.id} className="hover:bg-white/[0.02]">
                <td className="py-3 px-4">
                  <span className="font-semibold text-slate-200 block">{dom.name}</span>
                ...
                <td className="py-3 px-4 text-slate-400 text-[11px]">
                  ...
                  <span className="text-slate-500 block text-[10px]">Auto-renew enabled</span>
                </td>
                <td className="py-3 px-4">
                  {dom.linkedResourceId ? (
                    <span className="text-slate-200 flex items-center gap-1.5 font-mono text-[11px]">
                      <Server className="w-3.5 h-3.5 text-blue-400" />
                      {dom.linkedResourceId}
                    </span>
                  ) : (
                    <span className="text-slate-500 italic">Unlinked (Custom DNS)</span>
                  )}
                </td>
                <td className="py-3 px-4 font-mono">
                  <span className="text-slate-200 font-semibold block">${dom.retailAnnual}/yr</span>
      ...
      {activeDomain && (
        <div className="rounded-lg bg-[#161922] border border-[#232736] overflow-hidden p-6 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#232736]">
            <div>
              <h3 className="text-sm font-semibold text-slate-100 flex items-center gap-2">
                DNS Zone Editor: <span className="text-blue-400 font-mono">{activeDomain.name}</span>
              </h3>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400">1-Click Resource Pointer:</span>
              <select
                onChange={(e) => {
                  if (e.target.value) handle1ClickLink(e.target.value);
                }}
                className="h-8 px-2.5 rounded bg-[#11131A] border border-[#232736] text-xs text-slate-200"
                defaultValue=""
              >
          ...
          {/* Add Record Form */}
          <form onSubmit={handleAddRecord} className="p-3 bg-[#11131A] border border-[#232736] rounded-md grid grid-cols-5 gap-3 items-end">
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Type</label>
              <select
                value={recType}
                onChange={(e) => setRecType(e.target.value as any)}
                className="w-full h-8 px-2 rounded bg-[#161922] border border-[#232736] text-xs text-slate-200"
              >
            ...
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Hostname (@ / subdomain)</label>
              <input
                type="text"
                value={recName}
                onChange={(e) => setRecName(e.target.value)}
                className="w-full h-8 px-2 rounded bg-[#161922] border border-[#232736] text-xs font-mono text-slate-200"
                required
              />
            </div>
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Target Value / IP</label>
              <input
                type="text"
                value={recValue}
            ...
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">TTL (Seconds)</label>
              <input
                type="number"
                value={recTtl}
              ...
              {activeDomain.records.map((r) => (
                <tr key={r.id} className="hover:bg-white/[0.02]">
                  <td className="py-2.5 px-3">
                    <span className="text-[10px] font-semibold text-blue-400 bg-blue-600/10 px-1.5 py-0.5 rounded border border-blue-600/20">
                      {r.type}
                    </span>
                  </td>
                  <td className="py-2.5 px-3 text-slate-200">{r.name}</td>
                  <td className="py-2.5 px-3 text-slate-300 truncate max-w-xs">{r.value}</td>
      ...
      {isRegisterOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#161922] border border-[#232736] rounded-lg shadow-2xl w-full max-w-lg p-6 space-y-4">
            <h3 className="text-sm font-semibold text-slate-100">Register Top-Level Domain</h3>
            <div className="space-y-2">
              <label className="text-xs text-slate-400">Search Domain Name</label>
              <input
                type="text"
                value={searchDomain}
            ...
            <div className="space-y-2 pt-2">
              <span className="text-xs font-medium text-slate-300">Available TLDs:</span>
              <div className="space-y-1.5">
                {tldPrices.map((t) => (
                  <div
                    key={t.tld}
                    className="p-3 rounded bg-[#11131A] border border-[#232736] flex items-center justify-between"
                  >
                    <div>
                    ...
                    <button
                      ...
                      className="h-8 px-3 rounded bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-sm transition-colors"
                    >
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                className="h-8 px-4 rounded bg-[#1E2230] text-xs text-slate-300"
              >
                Close
              </button>
            </div>
