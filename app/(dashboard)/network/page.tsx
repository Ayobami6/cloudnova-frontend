...
export default function NetworkPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg bg-[#161922] border border-[#232736] overflow-hidden">
            <div className="p-4 border-b border-[#232736] flex items-center justify-between bg-[#11131A]">
              <div>
                <h3 className="text-sm font-semibold text-slate-200">{activeFirewall.name}</h3>
                <span className="text-[11px] text-slate-500">Default policy: Drop all unmatching ingress</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setIsAddRuleOpen(true)}
                  className="h-8 px-3 rounded bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold flex items-center gap-1.5"
                >
            ...
            <tbody className="divide-y divide-[#232736]">
              {loadBalancers.map((lb) => (
                <tr key={lb.id} className="hover:bg-white/[0.02]">
                  ...
                  <td className="py-3 px-4 font-mono text-slate-300">{lb.ip}</td>
                  <td className="py-3 px-4 font-mono text-slate-400">{lb.protocol} : {lb.port}</td>
                  <td className="py-3 px-4 capitalize text-slate-300">{lb.algorithm.replace("_", " ")}</td>
                  <td className="py-3 px-4">
                    <span className="text-emerald-400 text-xs">✓ Auto Let&apos;s Encrypt</span>
                  </td>
                  <td className="py-3 px-4 text-slate-400">{lb.targetInstanceIds.length} active backends</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody className="divide-y divide-[#232736]">
              {vpcs.map((v) => (
                <tr key={v.id} className="hover:bg-white/[0.02]">
                  <td className="py-3 px-4 font-semibold text-slate-200">{v.name}</td>
                  <td className="py-3 px-4 font-mono text-slate-400 uppercase">{v.region}</td>
                  <td className="py-3 px-4 font-mono text-slate-200">{v.cidr}</td>
                  <td className="py-3 px-4">
                    {v.isDefault ? (
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-blue-600/10 text-blue-400 border border-blue-600/20">
                        DEFAULT
                      </span>
                    ) : (
                  ...
                  <td className="py-3 px-4 text-slate-300">{v.connectedInstancesCount} Droplets</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#232736]">
              {floatingIps.map((fip) => {
                ...
                return (
                  <tr key={fip.id} className="hover:bg-white/[0.02]">
                    <td className="py-3 px-4 font-mono font-semibold text-slate-200">{fip.ip}</td>
                    <td className="py-3 px-4 font-mono uppercase text-slate-400">{fip.region}</td>
                    <td className="py-3 px-4">
                      {assigned ? (
                        ...
                        <span className="text-slate-500 italic">Unassigned (Standby pool)</span>
                      )}
                    </td>
                    <td className="py-3 px-4 text-right">
                      <button className="h-7 px-2.5 rounded bg-[#1E2230] hover:bg-[#252B3D] border border-[#232736] text-[11px] text-slate-300">
                        Reassign IP
                      </button>
                    </td>
                  </tr>
      ...
      {/* 5. DDoS Shield */}
      {activeTab === "ddos" && (
        <div className="p-6 rounded-lg bg-[#161922] border border-[#232736] space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4 text-xs">
            <div className="p-4 rounded bg-[#11131A] border border-[#232736]">
              <span className="text-slate-500 block">Threats Blocked (Last 24h)</span>
              <span className="text-xl font-mono font-semibold text-slate-100 mt-1 block">14,280</span>
            </div>
            <div className="p-4 rounded bg-[#11131A] border border-[#232736]">
              <span className="text-slate-500 block">Peak Mitigated Volume</span>
              <span className="text-xl font-mono font-semibold text-slate-100 mt-1 block">82.4 Gbps</span>
            </div>
            <div className="p-4 rounded bg-[#11131A] border border-[#232736]">
              <span className="text-slate-500 block">Scrubbing Latency Added</span>
              <span className="text-xl font-mono font-semibold text-emerald-400 mt-1 block">&lt; 1.2ms</span>
            </div>
          </div>
        </div>
      )}
      ...
      {isAddRuleOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#161922] border border-[#232736] rounded-lg shadow-2xl w-full max-w-md p-6 space-y-4">
            <h3 className="text-sm font-semibold text-slate-100">Add Firewall Rule</h3>
            <form onSubmit={handleAddRuleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs text-slate-300">Rule Label</label>
                <input
                  type="text"
                  value={ruleLabel}
              ...
              <div className="space-y-1.5">
                <label className="text-xs text-slate-300">Port Range (e.g. 80, 443, 8000-9000)</label>
                <input
                  type="text"
                  value={rulePorts}
