...
export function DeployDropletModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#161922] border border-slate-200 dark:border-[#232736] rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        {/* Modal Header */}
        <div className="h-14 bg-slate-50 dark:bg-[#11131A] border-b border-slate-200 dark:border-[#232736] px-6 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            <Server className="w-4 h-4 text-blue-600 dark:text-blue-400" />
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Provision New Compute Droplet</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Droplet Hostname</label>
            <input
              type="text"
              value={hostname}
              onChange={(e) => setHostname(e.target.value)}
              required
              className="w-full h-9 px-3 rounded-md bg-slate-50 dark:bg-[#11131A] border border-slate-200 dark:border-[#232736] text-xs font-mono text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500"
            />
          </div>

          {/* 1. Datacenter Region */}
          <div className="space-y-2">
            <label className="text-xs font-medium text-slate-700 dark:text-slate-300">1. Select Datacenter Region</label>
              ...
              {REGIONS.map((r) => (
                <button
                  ...
                  className={`p-2.5 rounded-md text-left transition-colors border ${
                    selectedRegion === r.id
                      ? "bg-blue-50 dark:bg-blue-600/12 border-blue-500 text-blue-900 dark:text-slate-100 shadow-xs"
                      : "bg-slate-50 dark:bg-[#11131A] border-slate-200 dark:border-[#232736] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:border-slate-300 dark:hover:border-[#33394D]"
                  }`}
                >
                  <div className="flex items-center justify-between text-xs">
                    <span>{r.flag} {r.name}</span>
                    <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">{r.pingMs}ms</span>
                  </div>
          ...
          {/* 2. OS Image */}
          <div className="space-y-2">
            <label className="text-xs font-medium text-slate-700 dark:text-slate-300">2. Operating System</label>
            <div className="grid grid-cols-4 gap-2">
              {OS_IMAGES.map((img) => (
                <button
                  key={img.id}
          ...
          {/* 3. Hardware Plan */}
          <div className="space-y-2">
            <label className="text-xs font-medium text-slate-700 dark:text-slate-300">3. Hardware Specification</label>
            <div className="space-y-1.5">
              {COMPUTE_PLANS.map((plan) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-xs font-semibold text-slate-800 dark:text-slate-200">{plan.name}</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Tags (comma-separated)</label>
              <input
                type="text"
                value={tags}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Assign to Client Sub-account</label>
              <select
          ...
          {/* Footer Submit */}
          <div className="pt-4 border-t border-slate-200 dark:border-[#232736] flex items-center justify-between">
            <div>
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onClose}
                className="h-9 px-3.5 rounded-md bg-slate-100 dark:bg-[#1E2230] hover:bg-slate-200 dark:hover:bg-[#252B3D] border border-slate-200 dark:border-[#232736] text-xs font-medium text-slate-700 dark:text-slate-300 transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
